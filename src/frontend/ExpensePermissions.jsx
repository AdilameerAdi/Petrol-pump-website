...
import { FaArrowLeft, FaMoneyBillWave, FaCheck, FaTimes } from "react-icons/fa";
...
export default function ExpensePermissions() {
  ...
  const fetchExpenseRequests = async () => {
    try {
      ...
      const expenseRequests = allRequests.filter(req => req.type === 'expense');
  ...
  const handleStatusUpdate = async (requestId, status) => {
    try {
      ...
      setMessage(`Expense request ${status} successfully!`);
      fetchExpenseRequests();
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        ...
        <span className="ml-3 text-gray-600">Loading expense requests...</span>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-gray-800">Expense Permissions</h1>
          </div>
          <p className="text-lg text-gray-600">Manage all expense requests</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          {filteredRequests.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              No expense requests found for the selected filter
            </div>
          ) : (
